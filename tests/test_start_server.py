...
def test_exported_project_root_survives(tmp_path):
    ...
    assert _launched_project_root([], {"DAEM0NMCP_PROJECT_ROOT": exported}) == exported
...
def test_batch_launcher_passes_the_callers_project(tmp_path, use_argument):
    ...
    project = tmp_path / "my project (x86)"
    project.mkdir()
    environment = {**os.environ, "PATH": f"{shim}{os.pathsep}{os.environ['PATH']}"}

    subprocess.run(
        ["cmd", "/c", str(REPO_ROOT / "start_daem0nmcp_server.bat")]
        + (["my project (x86)"] if use_argument else []),
